...
class StudioViewController: UIViewController, ErrorHandler {
    ...
    override func viewDidLoad() {
        ...
        self.txtLyrics.textContainerInset = UIEdgeInsets(top: 13, left: 0, bottom: 13, right: 13)

        let tap = UITapGestureRecognizer(target: self, action: #selector(didTapView(_:)))
        self.view.addGestureRecognizer(tap)
        
        NotificationCenter.default.addObserver(self, selector: #selector(audioRouteChangeListener), name: NSNotification.Name.AVAudioSession.routeChangeNotification, object: nil)
...
//            AKSettings.defaultToSpeaker = true
    ...
    func output(through port: AVAudioSession.PortOverride) {
